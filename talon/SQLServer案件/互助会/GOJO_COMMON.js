...
/**
 * 共通：安全に値を取得するヘルパー（これがないと .get() で落ちます）
 */
 ...
 */
function insertCom02(tkMap, SHORI_TUKI, sortKey, flg, kingaku, dataRow) {
    var db = TALON.getDbConfig();
    var pos = createPosHeader(tkMap, SHORI_TUKI, dataRow);
 ...
 * POSヘッダ項目生成
 */
function createPosHeader(tkMap, SHORI_TUKI, dataRow) {
    var db = TALON.getDbConfig();
    var USE_MONTH_END_FOR_PAYMENT = false;

    // 銀行情報 (getValueに変更)
    var whereBank = {
        BANK_CD: getValue(dataRow, "GINKOU_CD"),
        SHITEN_CD: getValue(dataRow, "SHITEN_CD")
    };
    ...
    if (!ymd) {
        ymd = USE_MONTH_END_FOR_PAYMENT ? endOfMonthYmd(SHORI_TUKI) : firstDayOfMonthYmd(SHORI_TUKI);
    }
    ...
    // SIMEI_KANA 決定（共通ロジック呼び出し）
    out["SIMEI_KANA"] = getKana(dataRow, tkMap, null);

    // 特別会員番号
    var rawTkNo = asStr(tkMap["TK_NO"]);
    out["TK_NO"] = rawTkNo ? zeroPad(rawTkNo, 5) : "";

    // 口座番号
    var yotakuKoza = asStr(getValue(dataRow, "KOUZA_NO"));
    var memberKoza = asStr(tkMap["KOUZA_NO"]);
    var koza = yotakuKoza ? yotakuKoza : memberKoza;
    ...
    // getValueに変更
    out["BANK_CD"] = getValue(dataRow, "GINKOU_CD");
    out["SHITEN_CD"] = getValue(dataRow, "SHITEN_CD");
 ...
 * 共通カナ決定ロジック
 */
function getKana(dataRow, tkMap, prefix) {
    // ① 預託連携データの「SIMEI_KANA」を優先
    var kanaYotaku = asStr(getValue(dataRow, "SIMEI_KANA"));
    if (kanaYotaku !== "" && kanaYotaku !== "0") return kanaYotaku;

    // ② 会員マスタ請求カナ
    var kanaSeikyu = tkMap ? asStr(tkMap["SEIKYU_KANA_NM"]) : "";
    if (kanaSeikyu !== "" && kanaSeikyu !== "0") return kanaSeikyu;

    // ③ 会員マスタ漢字名 fallback
    var kanjiSeikyu = tkMap ? asStr(tkMap["SEIKYU_KANJI_NM"]) : "";
    if (kanjiSeikyu !== "" && kanjiSeikyu !== "0") return kanjiSeikyu;
