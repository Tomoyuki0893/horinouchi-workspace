 ...
 * @param {*} initialJsonObj イニシャル情報
 * @returns 作成が不要な工程リスト
 */
function getNonCreateKouteiList(ankenObj, mitsumoriObj, hacchuJsonObj, initialJsonObj) {
    var nonCreateList = [];

    var mitsumoriRec = (mitsumoriObj && mitsumoriObj.record) ? mitsumoriObj.record : null;

    // 見積.field_1661157731_23(穴埋め) の値がなしの場合に工程順から140、141を自動削除
    if (mitsumoriRec && extractBeforeComma(mitsumoriRec.field_1661157731_23) == 'なし') {
        nonCreateList.push('140');
        nonCreateList.push('141');
    }
    // 見積.field_1661157731_22(TDR)の値がなしの場合に工程順から250を自動削除
    if (mitsumoriRec && extractBeforeComma(mitsumoriRec.field_1661157731_22) == 'なし') {
        nonCreateList.push('250');
    }
    // 見積.field_1673848989(シルクA)の値がなしかつ見積.field_1673849032(シルクB)の値がなしの場合に工程順からシルク工程を自動削除
    if (mitsumoriRec &&
        extractBeforeComma(mitsumoriRec.field_1673848989) == '000' &&
        extractBeforeComma(mitsumoriRec.field_1673849032) == '000') {
        nonCreateList.push('240');
    }
    // 見積.field_1673848759(レジストA)の値がなしかつ見積.field_1673848861(レジストB)の値がなしの場合に工程順からレジスト工程を自動削除
    if (mitsumoriRec &&
        extractBeforeComma(mitsumoriRec.field_1673848759) == '000' &&
        extractBeforeComma(mitsumoriRec.field_1673848861) == '000') {
        nonCreateList.push('230');
    }
    // 整面加工(field_1688888662)がなしの場合、面取り・バリ取り・Vカットを自動削除
    if (mitsumoriRec && extractBeforeComma(mitsumoriRec.field_1688888662) == 'なし') {
        nonCreateList.push('330'); // 面取り
        nonCreateList.push('340'); // バリ取り
        nonCreateList.push('350'); // Vカット
    }
 ...
 * 仕様書メインワークテーブルを作成する。
 * 
 * @param {Date} sysdate システム日時（呼び出し元で指定可。未指定なら内部で現在日時）
 * @param {String} hinmokuCd 品目コード
...
function _insertSiyosyoMainWork(
    ...
    // 日時は引数優先（なければ現在日時）
    var now = sysdate ? sysdate : new java.util.Date();

    // レコード存在チェック（落ちないように）
    var ankenRec = (ankenObj && ankenObj.record) ? ankenObj.record : null;
    var mitsumoriRec = (mitsumoriObj && mitsumoriObj.record) ? mitsumoriObj.record : null;
    var hacchuRec = (hacchuJsonObj && hacchuJsonObj.record) ? hacchuJsonObj.record : null;

    if (ankenRec) {
        // 得意先名
        TALON.getLogger().writeInfo(ankenRec.field_1661230909_1);
        // 得意先CD
        TALON.getLogger().writeInfo(ankenRec.field_1674115320);
    }

    if (mitsumoriRec) {
        TALON.getLogger().writeInfo(mitsumoriRec.field_1690858981);       // 品名
        TALON.getLogger().writeInfo(mitsumoriRec.field_1661158203);       // 表面処理
        TALON.getLogger().writeInfo(mitsumoriRec.field_1661157731_14);    // UL
        TALON.getLogger().writeInfo(mitsumoriRec.field_1661157731_22);    // インピーダンス測定
        TALON.getLogger().writeInfo(mitsumoriRec.field_1661157731_23);    // 穴埋め
        TALON.getLogger().writeInfo(mitsumoriRec.field_1661157731_24);    // 銅箔厚/内層
        TALON.getLogger().writeInfo(mitsumoriRec.field_1661157731_25);    // 銅箔厚/外層
        TALON.getLogger().writeInfo(mitsumoriRec.field_1661157731);       // 面付数
        TALON.getLogger().writeInfo(mitsumoriRec.field_1663130350);       // 面付管理
        TALON.getLogger().writeInfo(mitsumoriRec.field_1661157731_17);    // ランド径
        TALON.getLogger().writeInfo(mitsumoriRec.field_1661157731_18);    // 最小仕上径
        TALON.getLogger().writeInfo(mitsumoriRec.field_1661157731_7);     // 層数
        TALON.getLogger().writeInfo(mitsumoriRec.field_1673849341);       // 無電解金【Ni】
        TALON.getLogger().writeInfo(mitsumoriRec.field_1673849408);       // 電解金【Ni】
        TALON.getLogger().writeInfo(mitsumoriRec.field_1673849424);       // 電解金【Au】
        TALON.getLogger().writeInfo(mitsumoriRec.field_1685350584);       // ワークサイズ
        TALON.getLogger().writeInfo(mitsumoriRec.field_1692332127);       // 取り数
        TALON.getLogger().writeInfo(mitsumoriRec.field_1692668521);       // 品目コード(後)
        TALON.getLogger().writeInfo(mitsumoriRec.field_1690859781);       // 品目コード(前)
        TALON.getLogger().writeInfo(mitsumoriRec.field_1661157731_13);    // 基材グレード
        TALON.getLogger().writeInfo(mitsumoriRec.field_1688887352);       // 製品サイズ(X)
        TALON.getLogger().writeInfo(mitsumoriRec.field_1688888767);       // 製品サイズ(Y)
        TALON.getLogger().writeInfo(mitsumoriRec.field_1691634268);       // 面付数（別フィールド）
        TALON.getLogger().writeInfo(mitsumoriRec.field_1690751537);       // 仕上げ厚
    }

    if (hacchuRec) {
        TALON.getLogger().writeInfo(hacchuRec.field_1714012704);          // 基材メーカー
        TALON.getLogger().writeInfo(hacchuRec.field_1673848281);          // 板厚交差 プラス
        TALON.getLogger().writeInfo(hacchuRec.field_1713766844_72);       // 板厚交差 マイナス
        TALON.getLogger().writeInfo(hacchuRec.field_1714013554);          // 発注に関する備考
        TALON.getLogger().writeInfo(hacchuRec.field_1714013554);          // 専用伝票（※同じフィールドならこのログは重複）
        TALON.getLogger().writeInfo(hacchuRec.field_1714012314);          // 運送便
        TALON.getLogger().writeInfo(hacchuRec.field_1714012017);          // 専用伝票
        TALON.getLogger().writeInfo(hacchuRec.field_1713766844_62);       // 運送便
    }

    var userData = TALON.getUserInfoMap();
    var user_id = userData['USER_ID'];

    // ★ map は連想配列（オブジェクト）で作る（Arrayにしない）
    var map = {};
    ...
    if (
        ...
        var delSql = " DELETE FROM SIYO5001 WHERE SIYOSYO_NO = '" + siyosyoNo + "'";
        TalonDbUtil.delete(TALON.getDbConfig(), delSql);

        for (var i = 0; i < list.length; i++) {
            var rec = list[i].record;

            var qty = parseInt(rec.field_1738142279, 10);
            qty = isNaN(qty) ? 0 : qty;

            var unitPrice = parseInt(rec.field_1738142205, 10);
            ...
            var detailMap = {
                ...
                TANI: rec.field_1738142205, // ※元コード踏襲（単位フィールドが別ならここ要確認）
                QTY: qty,
    ...
    // =========================
    // map への詰め替え処理（SIYOSYO_MAIN_WORK）
    // =========================

    putMapValue(map, 'SIYOSYO_SEQ', getSiyoNum("1"));
    putMapValue(map, 'SIYOSYO_NO', siyosyoNo);
    putMapValue(map, 'HINMOKU_CD', hinmokuCd ? hinmokuCd.toUpperCase() : null);

    putMapValue(map, 'HIN_NM', mitsumoriRec ? mitsumoriRec.field_1690858981 : null);
    putMapValue(map, 'WORK_SIZE', convertWorkSizeToCode(mitsumoriRec ? mitsumoriRec.field_1685350584 : null));
    putMapValue(map, 'SOUYUKOU_KIKAN', '12');

    putMapValue(
        map,
        'HYOMEN_SYORI_CD',
        convertHyomenToCode(getJustKomokuCode(mitsumoriRec, "field_1661158203"))
    );

    putMapValue(map, 'USER_NM', toHankakuKana(hacchuRec ? hacchuRec.field_1713766844_17 : null));

    /* ===== 数値項目：null → 0 ===== */
    putMapValue(
        map,
        'SOUSU',
        nvlNumber(extractNumberBeforeComma(mitsumoriRec ? mitsumoriRec.field_1661157731_7 : null))
    );
    putMapValue(map, 'MENTUKESU1', nvlNumber(mitsumoriRec ? mitsumoriRec.field_1661157731 : null));
    putMapValue(map, 'MENTUKESU2', nvlNumber(mitsumoriRec ? mitsumoriRec.field_1691634268 : null));
    putMapValue(map, 'ULNO', convertULNOToCode(mitsumoriRec ? mitsumoriRec.field_1661157731_14 : null));
    putMapValue(map, 'TORISU', nvlNumber(mitsumoriRec ? mitsumoriRec.field_1692332127 : null));

    putMapValue(map, 'SEIHIN_SIZE_TATE', nvlNumber(mitsumoriRec ? mitsumoriRec.field_1688887352 : null));
    putMapValue(map, 'SEIHIN_SIZE_TATE_P', 0);
    putMapValue(map, 'SEIHIN_SIZE_TATE_M', 0);

    putMapValue(map, 'SEIHIN_SIZE_YOKO', nvlNumber(mitsumoriRec ? mitsumoriRec.field_1688888767 : null));
    putMapValue(map, 'SEIHIN_SIZE_YOKO_P', 0);
    putMapValue(map, 'SEIHIN_SIZE_YOKO_M', 0);

    /* ===== 外装・内装 ===== */
    var gaisoAry = mitsumoriRec ? mitsumoriRec.field_1661157731_25 : null;
    var naisouAry = mitsumoriRec ? mitsumoriRec.field_1661157731_24 : null;

    var gaisoVal = (Array.isArray(gaisoAry) && gaisoAry.length > 0)
        ? convertNaisoGaiso(gaisoAry[0])
        : null;

    var naisouVal = (Array.isArray(naisouAry) && naisouAry.length > 0)
        ? convertNaisoGaiso(naisouAry[0])
        : null;

    putMapValue(map, 'GAISOU', gaisoVal);
    putMapValue(map, 'NAISOU1', naisouVal);

    // ★ 数値扱いなら null ではなく 0 に統一（要件に合わせて）
    putMapValue(map, 'NAISOU2', 0);
    ...
    putMapValue(
        map,
        'KIZAI_MAKER',
        convertKizaiMakerToCode(extractNumberBeforeColon(hacchuRec ? hacchuRec.field_1714012704 : null))
    );

    // ★ 配列アクセスは null/空配列だと落ちるので防御
    var kizaiGradeSrc = (mitsumoriRec && mitsumoriRec.field_1661157731_13 && Array.isArray(mitsumoriRec.field_1661157731_13) && mitsumoriRec.field_1661157731_13.length > 0)
        ? mitsumoriRec.field_1661157731_13[0]
        : null;

    putMapValue(map, 'KIZAI_GRADE', convertKizaiGradeToCode(kizaiGradeSrc));

    putMapValue(map, 'BIKO', hacchuRec ? hacchuRec.field_1714013554 : null);
    putMapValue(map, 'COPY_FLG', "0");
    putMapValue(map, 'KOKIBANSU', "0");
    putMapValue(map, 'TRHK_CD', ankenRec ? ankenRec.field_1674115320 : null);
    putMapValue(map, 'SOU_HIT', 0);
    putMapValue(map, 'MINI_VIA', 0);
    putMapValue(map, 'MIN_SIAGE_KEI', 0);
    putMapValue(map, 'NAISOU_LINE_HABA', 0);
    putMapValue(map, 'GAISOU_LINE_HABA', 0);
    putMapValue(map, 'LINE_KANKAKU', 0);
    putMapValue(map, 'LINE_RANDO', 0);
    putMapValue(map, 'RANDO_KEI', 0);
    putMapValue(map, 'IVH_FLG', 0);
    putMapValue(map, 'IVH_NAISO_FLG', 0);

    /* ===== 金端子 ===== */
    putMapValue(
        map,
        'KIN_TANSHI',
        getJustKomokuCode(mitsumoriRec, "field_1689693307") === "なし" ? "0" : "1"
    );
