...
(function (NS) {
    ...
    NS._sendEngine = function (config) {
        ...
        // ▼ここから追加（Google Workspace IPリレー対策）─────────────
        // HELO/EHLO で送信元の登録ドメインを提示する
        var fromDomain = String(config.from || '').split('@')[1];
        if (fromDomain) {
            props.put("mail.smtp.localhost", fromDomain.trim());  // → "virbac.co.jp"
        }
        // envelope-from を明示（空 envelope-from 対策）
